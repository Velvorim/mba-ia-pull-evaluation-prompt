...
def extract_json_from_response(response_text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[AVISO] Nao foi possivel extrair JSON da resposta: {response_text[:200]}...")
...
def evaluate_tone_score(bug_report: str, user_story: str, reference: str, run_name: str = "") -> Dict[str, Any]:
    """
...
"""
    ...
    try:
        ...
        response = llm.invoke([HumanMessage(content=evaluator_prompt)], config=config)
        result = extract_json_from_response(response.content)
...
def evaluate_acceptance_criteria_score(bug_report: str, user_story: str, reference: str, run_name: str = "") -> Dict[str, Any]:
    """
...
def evaluate_user_story_format_score(bug_report: str, user_story: str, reference: str, run_name: str = "") -> Dict[str, Any]:
    """
...
Avalie APENAS o FORMATO da user story gerada (estrutura, template, persona, ação, benefício, separação de seções). Não penalize por escopo ou tamanho: uma história que cubra vários problemas do mesmo relato ainda pode ter formato correto; questões de "Story Épica" ou INVEST são de conteúdo/completude, não de formato.
...
def evaluate_completeness_score(bug_report: str, user_story: str, reference: str, run_name: str = "") -> Dict[str, Any]:
    """
...
- E o contador do carrinho deve ser atualizado"""
    ...
    test_reference_story = test_user_story
